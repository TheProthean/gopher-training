...
//MakeDecision ...
func (ai *AI) MakeDecision(state blackjack.RoundState, expandedOptions bool) blackjack.AIDecision {
	AIScore := blackjack.CountScore(state.AICards)
	if expandedOptions {
		if shouldDoubleDown(AIScore, state.DealerCard) {
			return blackjack.DOUBLEDOWN
		}
		//if shouldSplit(state.AICards) {
...
func shouldDoubleDown(score int, dealerCard deck.Card) bool {
	return score == 11 || (score == 10 && !(dealerCard.Value >= deck.JACK || dealerCard.Value == deck.ACE)) || (score == 9 && !(dealerCard.Value >= deck.SEVEN || dealerCard.Value == deck.ACE))
}

func shouldSplit(hand []deck.Card) bool {
	return hand[0].Value == hand[1].Value && (hand[0].Value == deck.ACE || hand[0].Value == deck.EIGHT)
}
